...
TG_BOT_TOKEN="5305982215:AAFuXmUHa8sHPFkKDgAKVJwjvJGvmRBgqvw"
TG_CHAT_ID="-1001764856647"
...
export DTC_EXT=dtc
export PATH="${CLANG_DIR}:${GCC_DIR}:${GCC32_DIR}:$PATH"
make O=out ARCH=arm64 $DEFCONFIG
#export LD_LIBRARY_PATH="/home/ste0090/zep/prebuilts/clang/host/linux-x86/clang-liyuu/lib:$LD_LIBRARY_PATH" \
}

kmake() {
  make                O=out -j$(nproc --all) Image dtbo.img dtb.img \
	       ARCH=arm64 \
	       LLVM=1 \
	       LLVM_IAS=1 \
...
kcopy() {
cp out/arch/arm64/boot/dtbo.img ./../../zip/Perrythekernel/
cp out/arch/arm64/boot/dtb.img ./../../zip/Perrythekernel/
cp out/arch/arm64/boot/Image ./../../zip/Perrythekernel/
}
